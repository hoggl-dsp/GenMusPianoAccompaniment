...
with col1:
    ...
    onset_algorithm = st.selectbox(
        "Onset algorithm",
        ["Complex Domain", "High Frequency Content", "Spectral Difference", "Phase Deviation"],
        index=0
    )
...
if st.button("🎵 Generate!", type="primary"):
    if st.session_state.uploaded_file is not None:
        with st.spinner("Processing audio... This may take a moment."):
            ...
            generated_audio = generated_audio * dry_wet + np.random.normal(0, 0.1, len(generated_audio)) * (1 - dry_wet)
