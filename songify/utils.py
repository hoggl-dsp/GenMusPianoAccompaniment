...
def synthesise_score(
    score: symusic.types.Score, sample_rate: int = 44100
) -> torch.Tensor:
    """
    Synthesize the given symusic Score into audio (as a torch Tensor).
    """
    return torch.from_numpy(
        symusic.Synthesizer(sample_rate=sample_rate).render(score=score, stereo=True)
    )
...
def mix_audio(
    ...
    if original_audio.size(1) < synthesized_audio.size(1):
        original_audio = torch.cat(
            (
                original_audio,
                torch.zeros((2, synthesized_audio.size(1) - original_audio.size(1))),
            ),
            dim=-1,
        )
    elif original_audio.size(1) > synthesized_audio.size(1):
        synthesized_audio = torch.cat(
            (
                synthesized_audio,
                torch.zeros((2, original_audio.size(1) - synthesized_audio.size(1))),
            ),
            dim=-1,
        )
